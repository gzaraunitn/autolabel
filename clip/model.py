...
class Transformer(nn.Module):
    def __init__(
        ...
        self.resblocks = nn.Sequential(
            *[
                ResidualAttentionBlock(width, heads, attn_mask, dropout=dropout[i])
...
class CLIP(nn.Module):
    ...
    def encode_text_with_prompts(self, prompts, tokenized_prompts):
...
def build_model(
    ...
    if pretrain:
        print(colored("Loading clip pretrained model...", "green"))
        if joint:  # or emb_dropout>0 or dropout>0
            model.load_state_dict(state_dict, strict=False)
        else:
            model.load_state_dict(state_dict)
        print(colored("Clip pretrained model loaded", "green"))
    else:
        print(colored("Not using full clip pretrained model, only visual!", "yellow"))
